...
pub trait Device<B: Backend>: fmt::Debug + Any + Send + Sync {
    ...
    unsafe fn set_image_name(&self, image: &mut B::Image, name: &str) -> Result<(), OutOfMemory>;
    /// Associate a name with a buffer, for easier debugging in external tools or with validation
    /// layers that can print a friendly name when referring to objects in error messages
    unsafe fn set_buffer_name(&self, buffer: &mut B::Buffer, name: &str) -> Result<(), OutOfMemory>;
    ...
    /// layers that can print a friendly name when referring to objects in error messages
    unsafe fn set_fence_name(&self, fence: &mut B::Fence, name: &str) -> Result<(), OutOfMemory>;
